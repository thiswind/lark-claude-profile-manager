...
class VercelProvider(IntegrationProvider):
    ...
    def check_host(self) -> HostCheck:
        ...
        if token:
            ...
            return HostCheck(
                ...
                message="vercel token authenticated",
                details={"account": whoami.stdout.strip(), "auth": "token"},
            )
    ...
    def verify_commands(self, profile: Profile, external: bool = False) -> list[str]:
        ...
        token_file = f"{home}/.local/share/com.vercel.cli/lcp-token.json"
        return [
            f"tmp=$(mktemp -d); if [ -s {shlex.quote(token_file)} ]; then token=$(node -e 'process.stdout.write(JSON.parse(require(\"fs\").readFileSync(process.argv[1], \"utf8\")).token)' {shlex.quote(token_file)}) && HOME=\"$tmp\" vercel whoami --token \"$token\"; code=$?; rm -rf \"$tmp\"; exit $code; else mkdir -p \"$tmp/.local/share\" && cp -a {home}/.local/share/com.vercel.cli \"$tmp/.local/share/\" && chmod -R u+w \"$tmp/.local/share/com.vercel.cli\" && HOME=\"$tmp\" vercel whoami; code=$?; rm -rf \"$tmp\"; exit $code; fi"
        ]
    ...
    def _write_token_auth_snapshot(self, path: Path, token: str) -> None:
        ...
        (path / "lcp-token.json").write_text(json.dumps({"token": token}) + "\n", encoding="utf-8")
        (path / "lcp-token.json").chmod(0o600)
