...
默认使用 `pip` 从 GitHub 源码安装。建议安装发布标签版本，例如当前版本 `0.2.5`：

```bash
python -m pip install --user 'lark-claude-profile-manager @ git+https://github.com/thiswind/lark-claude-profile-manager.git@0.2.5'
```
...
```bash
uv tool install 'lark-claude-profile-manager @ git+https://github.com/thiswind/lark-claude-profile-manager.git@0.2.5'
pipx install 'lark-claude-profile-manager @ git+https://github.com/thiswind/lark-claude-profile-manager.git@0.2.5'
```
...
- `image build-base` 管理 LCP 共享基础镜像，默认 tag 使用 LCP 版本和 Ubuntu LTS 后缀，例如 `lcp/base:0.2.5-ubuntu24.04`。
- `runtime apply` 构建 LCP runtime 镜像，默认 tag 同样版本化，例如 `lcp/runtime:0.2.5-ubuntu24.04`，不会自动重建现有 profile 容器。
