...
- `proxy`：把显式提供的 HTTP、HTTPS、SOCKS 代理配置写入容器，并生成 profile-local Claude Code proxy skill。
...
- `proxy` 只从显式环境变量读取代理地址；LCP 不内置任何宿主机特定代理端点。可用变量包括 `LCP_PROXY_HTTP`、`LCP_PROXY_HTTPS`、`LCP_PROXY_SOCKS5`、`LCP_PROXY_NO_PROXY`。
- `proxy` apply 会写入 `/etc/profile.d/lcp-proxy.sh`、apt/npm/pip 配置，并生成 `~/.claude/skills/lcp-proxy-<profile>/SKILL.md`。
