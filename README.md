...
默认使用 `pip` 从 GitHub 源码安装。建议安装发布标签版本，例如当前版本 `0.2.4`：

```bash
python -m pip install --user 'lark-claude-profile-manager @ git+https://github.com/thiswind/lark-claude-profile-manager.git@0.2.4'
```
...
```bash
uv tool install 'lark-claude-profile-manager @ git+https://github.com/thiswind/lark-claude-profile-manager.git@0.2.4'
pipx install 'lark-claude-profile-manager @ git+https://github.com/thiswind/lark-claude-profile-manager.git@0.2.4'
```
...
lcp profile cleanup-rollbacks <name> --dry-run
lcp profile cleanup-rollbacks <name> --yes
...
- 真实重建必须先显式传入 `--yes`，建议始终先看 `--dry-run`。
- 重建前会检查宿主机 Claude Code 配置和 `~/.claude/projects` 是否能继续挂载。
- 旧容器会改名保留为 rollback 容器；新容器继续使用原容器名。
- 重建后会重新 apply 当前启用的 integrations。
...
- `image build-base` 管理 LCP 共享基础镜像，默认 tag 使用 LCP 版本和 Ubuntu LTS 后缀，例如 `lcp/base:0.2.4-ubuntu24.04`。
- `runtime apply` 构建 LCP runtime 镜像，默认 tag 同样版本化，例如 `lcp/runtime:0.2.4-ubuntu24.04`，不会自动重建现有 profile 容器。
